...
import { generatePoemFromImage, GeneratePoemFromImageInput } from '@/ai/flows/generate-poem-from-image';
import { generateSongFromPoem, GenerateSongFromPoemInput } from '@/ai/flows/generate-song-from-poem';
...
import { Loader2, Upload, Image as ImageIcon, Music, Play, FileWarning, Info } from 'lucide-react';
...
export default function PictoPoemPage() {
  ...
  const handleImageChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      setError(`Invalid file type. Please upload an image (JPG, PNG, GIF, etc.).`);
      toast({
    ...
    if (file.size > MAX_IMAGE_SIZE_BYTES) {
       setError(`Image too large. Please upload an image smaller than ${MAX_IMAGE_SIZE_MB}MB.`);
       toast({
    ...
    fetch(sampleImageUrl)
      ...
      .then(blob => {
          if (blob.size > MAX_IMAGE_SIZE_BYTES) {
             throw new Error(`Sample image is too large (${(blob.size / 1024 / 1024).toFixed(1)}MB). Max ${MAX_IMAGE_SIZE_MB}MB allowed.`);
          }
          if (!blob.type.startsWith('image/')) {
             throw new Error(`Sample image is not a valid image type (${blob.type}).`);
          }
      ...
      .then(dataUri => {
        ...
        setImageDataUri(dataUri);
        toast({ title: 'Sample Image Loaded', description: 'Ready to generate poem.'});
      })
      .catch(error => {
        console.error("Error fetching/converting sample image:", error);
        const message = error instanceof Error ? error.message : "Could not load the sample image data.";
        ...
        <form onSubmit={handlePoemSubmit}> {/* Wrap relevant parts in form */}
            ...
            <div className="space-y-4 flex flex-col">
                <section aria-labelledby="image-section-title">
                    <h2 id="image-section-title" className="text-xl font-semibold text-foreground mb-2">1. Choose Your Image</h2>
                    ...
                    {imagePreview ? (
                        ...
                        <p className="text-xs mt-1">(Max {MAX_IMAGE_SIZE_MB}MB)</p>
                        </div>
                    )}
                    </div>
                    <div className="flex flex-col sm:flex-row gap-2 mt-3">
                        <Label htmlFor="image-upload" className="flex-1 cursor-pointer">
                            <Button asChild variant="outline" className="w-full" aria-controls="image-upload">
                        ...
                        <Button variant="secondary" onClick={handleUseSampleImage} type="button" className="flex-1">
                            Use Sample Image
                        </Button>
                    </div>
                </section>

                <section aria-labelledby="style-section-title" className="pt-4 flex-grow flex flex-col">
                    <h2 id="style-section-title" className="text-xl font-semibold text-foreground mb-2">2. Describe Style (Optional)</h2>
                     <Label htmlFor="style-description" className="sr-only">Poem and Song Style Description</Label>
                    <Textarea
                    ...
                    />
                    <p className="text-xs text-muted-foreground mt-1">Influences poem structure and song mood/genre.</p>
                </section>
            </div>
            ...
            <div className="space-y-6 flex flex-col">
                ...
                <section aria-labelledby="poem-generation-title" className="flex flex-col">
                    ...
                    <div className="mt-4 flex-grow flex flex-col">
                        <h3 className="text-lg font-medium text-foreground mb-2 sr-only">Generated Poem Output</h3>
                        <div className={`bg-card border p-4 rounded-md shadow-inner min-h-[150px] flex-grow flex ${!poem && !isPoemPending ? 'items-center justify-center' : ''} overflow-auto`}>
                        {isPoemPending ? (
                            <div className="text-center text-muted-foreground p-4 flex items-center justify-center w-full">
                             ...
                             <p className="whitespace-pre-wrap font-serif text-sm md:text-base text-card-foreground">{poem}</p>
                        ) : imageDataUri ? (
                ...
                <section aria-labelledby="song-generation-title" className="flex flex-col">
                    ...
                    {/* Song Player */}
                    <div className="mt-4 flex-grow flex flex-col">
                        <h3 className="text-lg font-medium text-foreground mb-2 sr-only">Generated Song Player</h3>
                         <div className={`bg-card border p-4 rounded-md shadow-inner min-h-[80px] flex-grow flex items-center justify-center ${!songUrl && !isSongPending ? 'items-center justify-center' : ''} overflow-hidden`}>
                            {isSongPending ? (
                                <div className="text-center text-muted-foreground p-4 flex items-center justify-center w-full">
                                ...
                                <div className="w-full space-y-2">
                                     <h4 className="text-base font-medium flex items-center"><Play className="mr-2 h-5 w-5 text-accent"/> Song Player</h4>
                                    <audio ref={audioRef} controls src={songUrl} className="w-full h-10" aria-label="Generated song audio player">
                                        Your browser does not support the audio element.
                                        <a href={songUrl} download target="_blank" rel="noopener noreferrer" className="underline ml-2">Download Song</a>
                                    </audio>
                                </div>
                            ) : poem ? (
                                 <div className="text-center text-muted-foreground p-4 w-full">
